...
class PDFDataExtractor(LoggerMixin):
    ...
    def extract_data(self, pdf_path: str) -> ExtractedData:
        """Extrai todos os dados relevantes do PDF com fallback OCR inteligente"""
        ...
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                extracted_data = self._parse_text_data(full_text, pdf_path) if len(full_text.strip()) >= 100 else None
                if extracted_data and self._is_extraction_sufficient(extracted_data): 
                    return extracted_data
            
            if OCR_AVAILABLE:
                ocr_text = self._extract_text_with_ocr(pdf_path)
                ocr_extracted_data = self._parse_text_data(ocr_text, pdf_path)
    ...
    def _parse_text_data(self, text: str, pdf_path: str = None) -> ExtractedData:
        try:
            from .pp_extractor import ProposalExtractor
            proposal_extractor = ProposalExtractor()
            extracted_data = proposal_extractor.extract_proposal_data(text, pdf_path)
            if extracted_data and self._is_extraction_sufficient(extracted_data):
                self.logger.info("Using ProposalExtractor for vehicle proposal data")
                return extracted_data
        except Exception as e:
            self.logger.warning(f"ProposalExtractor failed, using fallback: {e}")
        ...
        # CKDEV-NOTE: Não extrair dados de pagamento da proposta - esses dados devem vir apenas do comprovante de pagamento
        # payment = self._extract_payment_data(text)
        new_vehicle = self._extract_new_vehicle_data(text)
        return ExtractedData(client=client, vehicle=vehicle, document=document, payment=None, new_vehicle=new_vehicle, third_party=None)
    ...
    def _extract_client_data(self, text: str) -> ClientData:
        ...
        name_patterns = [
            r'Cliente:\s*\n?\s*([A-ZÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ\s]+?)(?:\s*CNPJ/CPF:|\s*Endereço:|\n)',
        ...
        for pattern in name_patterns:
            ...
            if name_match:
                ...
                if not any(word in name.upper() for word in ['TELEFONICA', 'BRASIL', 'S.A.', 'LTDA', 'ENGENHEIRO', 'AVENIDA', 'RUA']):
                    client.name = name
                    break
        
        cpf_matches = self.patterns['cpf'].findall(text)
        if cpf_matches: 
            client.cpf = cpf_matches[0]
        
        rg_patterns = [
            r'Ident/Inscrição:\s*([0-9]{1,2}\.?[0-9]{3}\.?[0-9]{3})', 
            r'Ident/Inscrição:\s*([0-9]+(?:\.[0-9]+)*)', 
            r'RG[:\s]*([0-9]{1,2}\.?[0-9]{3}\.?[0-9]{3})', 
            r'Identidade[:\s]*([0-9]{1,2}\.?[0-9]{3}\.?[0-9]{3})'
        ]
        ...
        fatura_pattern = r'([A-Z\s]+)\n(Rua\s*-\s*[A-Za-z\s]+)\s+(\d+)\n([A-Z\s]+?)\s*(?:\d[^\n]*)\n(\d{5}-\d{3})\s+([A-Z\s]+)\s*-\s*SP'
        fatura_match = re.search(fatura_pattern, text, re.IGNORECASE | re.DOTALL)
        
        if fatura_match:
            street_info['logradouro'] = fatura_match.group(2).replace('-', '').strip().upper()
            street_info['numero'] = fatura_match.group(3).strip()
            bairro_raw = fatura_match.group(4).strip().upper()
        ...
        if fatura_match:
            client.city = fatura_match.group(6).strip().upper()
            client.cep = fatura_match.group(5)
        else:
            city_patterns = [
    ...
    def _extract_document_data(self, text: str) -> DocumentData:
        ...
        if proposal_match: 
            document.proposal_number = proposal_match.group(1)
        ...
        if not document.date:
            date_patterns = [
                r'Emissão da Proposta:\s*(\d{2}/\d{2}/\d{4})', 
                r'(\d{1,2})\s*de\s*([A-ZÁÀÂÃÉÈÊÍÏÓÔÕÖÚÇÑa-záàâãéèêíïóôõöúçñ]+)\s*de\s*(\d{4})', 
                r'\b(\d{2}/\d{2}/\d{4})\b'
            ]
            ...
            if not document.date:
                ...
                if date_matches: 
                    document.date = date_matches[-1] if len(date_matches) > 1 else date_matches[0]
    ...
    def _extract_new_vehicle_data(self, text: str) -> Optional[NewVehicleData]:
        """Extrai dados do veículo novo sendo adquirido"""
        new_vehicle = NewVehicleData()
        
        vehicle_section = re.search(r'IDENTIFICAÇÃO DO VEÍCULO.*?(?=DESCRIÇÃO|$)', text, re.DOTALL)
        
        # CKDEV-NOTE: Se não encontrar a seção específica, tentar buscar em todo o texto
        if not vehicle_section:
            vehicle_section = re.search(r'VEÍCULO.*?(?=DESCRIÇÃO|VALOR|TOTAL|$)', text, re.DOTALL | re.IGNORECASE)
        ...
        if vehicle_section:
            ...
            if model_match: 
                new_vehicle.model = model_match.group(1).strip()
            
            brand_match = re.search(r'Marca:\s*\n?\s*([A-Z\s]+)', section_text)
            if brand_match: 
                brand_raw = brand_match.group(1).strip()
                if brand_raw == 'VOLKSWAGEN N':
                    new_vehicle.brand = 'VOLKSWAGEN'
                else:
                    new_vehicle.brand = brand_raw
            else:
                # CKDEV-NOTE: Tentar regex alternativo para marca
                brand_match_alt = re.search(r'Marca\s*:\s*([A-Z\s]+)', section_text, re.IGNORECASE)
                if brand_match_alt:
                    brand_raw = brand_match_alt.group(1).strip().upper()
                    if brand_raw == 'VOLKSWAGEN N':
                        new_vehicle.brand = 'VOLKSWAGEN'
                    else:
                        new_vehicle.brand = brand_raw
            ...
            for pattern in color_patterns:
                ...
                if color_match: 
                    ...
                    if color_clean.endswith(' A'):
                        color_clean = color_clean[:-2].strip()
            ...
            if year_match: 
                new_vehicle.year_model = year_match.group(1).replace(' ', '')
            
            chassis_match = re.search(r'Chassi:\s*\n?\s*([A-Z0-9]{17})', section_text)
            if chassis_match: 
                new_vehicle.chassis = chassis_match.group(1)
        
        value_patterns = [r'Subtotal:\s*\n?\s*([\d.,]+)', r'Total:\s*\n?\s*([\d.,]+)', r'Veículo:\s*\n?\s*([\d.,]+)']
        for pattern in value_patterns:
            match = re.search(pattern, text)
            if match: 
                new_vehicle.value = match.group(1)
                break
        
        # CKDEV-NOTE: Retorna se tiver pelo menos marca OU modelo (mais permissivo)
        return new_vehicle if (new_vehicle.model or new_vehicle.brand or new_vehicle.year_model or new_vehicle.chassis) else None
    ...
    def _clean_vehicle_model(self, raw_model: str) -> str:
        ...
        corrections = {
            'FLEXPRETO': 'FLEX', 'FLEXBRANCO': 'FLEX', 'FLEXPRATA': 'FLEX', 
            'AUTOPRETO': 'AUTOMATICO', 'AUTOBRANCO': 'AUTOMATICO', 'AUTOPRATA': 'AUTOMATICO'
        }
        for wrong, correct in corrections.items():
            if wrong in model: 
                model = model.replace(wrong, correct)
        
        if 'FLEXP' in model and 'RPERTEO' in model: 
            model = re.sub(r'FLEXP\s+RPERTEO.*', 'FLEX', model)
        ...
        ocr_corrections = {
            r'\bFRLAENXC\b': 'FLEX', r'\bFLEXP\b': 'FLEX', r'\bTOTALB\b': 'TOTAL', 
            r'\bR-LINE\b': 'R-LINE', r'\bTIPT\b': 'TIPT', r'\b1\.4\b': '1.4', 
            r'\b250\b': '250', r'\bTSI\b': 'TSI', r'\bAUTOMATI.*\b': 'AU'
        }
        for wrong, correct in ocr_corrections.items(): 
            model = re.sub(wrong, correct, model, flags=re.IGNORECASE)
        ...
        known_models = {
            r'^JETTA.*1\.4.*250.*TSI.*': 'JETTA 1.4 250 TSI', 
            r'^NIVUS.*HIGHLINE.*200.*TSI.*': 'NIVUS HIGHLINE 200 TSI', 
            r'^T-?CROSS.*200.*TSI.*': 'T-CROSS 200 TSI', 
            r'^VIRTUS.*1\.0.*200.*TSI.*': 'VIRTUS 1.0 200 TSI', 
            r'^POLO.*1\.0.*200.*TSI.*': 'POLO 1.0 200 TSI', 
            r'^UP.*1\.0.*TSI.*': 'UP 1.0 TSI'
        }
    ...
    def _extract_brand_from_model(self, model: str) -> str:
        """Extrai a marca do modelo do veículo usando o dicionário CSV"""
        if not model: 
            return ""
